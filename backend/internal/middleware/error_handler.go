...
// ErrorHandler 全局错误恢复与统一错误响应（recovery + 错误码映射）。
	...
	return func(c *gin.Context) {
		...
		for _, err := range c.Errors {
			...
			if errors.As(err.Err, &verrs) {
				msg := fmt.Sprintf(constants.MsgValidationFailed, verrs[0].Field(), verrs[0].Tag())
...
func httpStatusOf(code int) int {
	...
	case constants.CodeConflict, constants.CodeInvalidStatus, constants.CodePrescriptionLocked, constants.CodeOverrideNotAllowed:
		return http.StatusConflict
	case constants.CodeValidation, constants.CodeBadRequest, constants.CodeInvalidFormat, constants.CodeDrugNotFound:
